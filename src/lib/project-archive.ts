...
import { canBulkDisposeTask } from './task-decisions'
...
function isActiveTask(task: Task): boolean {
  return task.completedAt == null && task.checked !== true && task.isDeleted !== true
}

function createsParentCycle(
  taskId: string,
  parentId: string,
  nodes: Map<string, ProjectTaskNode>,
): boolean {
...
/** Builds a forest from every active task, preserving input order at each level. */
export function buildProjectTaskTree(tasks: Task[]): ProjectTaskNode[] {
  const nodes = new Map<string, ProjectTaskNode>()

  for (const task of tasks) {
    if (!isActiveTask(task) || nodes.has(task.id)) continue
  ...
  for (const node of nodes.values()) {
    ...
    if (!parentId || !parent || createsParentCycle(node.task.id, parentId, nodes)) {
      roots.push(node)
      continue
    }
...
/**
 * Splits top-level task branches into branches that can be disposed through
 * their root and branches that must remain untouched because they contain a
 * Recurring task.
 */
export function buildProjectArchivePlan(tasks: Task[]): ProjectArchivePlan {
  ...
  const recurringTaskCount = recurringBranches.reduce(
    (total, branch) => total + branch.recurringTaskCount,
    0,
  )

  return {
    roots,
    activeTaskCount: eligibleTaskCount + preservedTaskCount,
    eligibleTaskCount,
    preservedTaskCount,
    recurringTaskCount,
    eligibleBranches,
...
/**
 * Attempts every eligible root operation before archiving the project. Each
 * rejected task operation is counted and does not prevent later branches or
 * the Project archive from being attempted.
 */
export async function archiveProjectWithTaskDisposition(
  ...
  const result: ProjectArchiveResult = {
    ...
    keptOpenTasks: input.choice === 'keep_open'
      ? plan.activeTaskCount
      : plan.preservedTaskCount,
    completedTasks: 0,
    deletedTasks: 0,
  ...
  if (input.choice !== 'keep_open') {
    for (const branch of plan.eligibleBranches) {
      try {
  ...
  try {
    await api.archiveProject(input.projectId)
