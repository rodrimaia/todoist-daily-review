...
A Delivery Slice is an independently deliverable vertical outcome represented by exactly one standard OpenSpec Spec Change, one generated Agent-Ready Issue, and one atomic default-branch commit.

Accept a slice only when:

- its commit leaves the repository's default branch, the code, and the Spec Corpus internally consistent;
